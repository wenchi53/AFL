...
/*
  ...
  fname = alloc_printf("%s/filettt.txt", out_dir);
  ...
  close(fd);
    ...
    u8* fname;
    s32 fd;
    fname = alloc_printf("%s/queue", out_dir);
  ...
  FILE *f = fopen("./o/output/file.txt", "w");
  if (f == NULL){
...
*/
  while (q) {
    //ck_write(fd, mem, q->len, fname);
    q->favored = 0;
    //fprintf(f, "The name is: %s\n", );
    q = q->next;
...
/*
   ...
   FILE* file = fopen ("./o/message/message.txt", "r");
   int number;
